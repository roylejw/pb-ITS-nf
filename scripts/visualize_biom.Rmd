...
if(params$merged_tax_tab_file != ""){
  ...
  colnames(tax_table(phyloseq))= c("Kingdom","Phylum","Class","Order","Family","Genus", "Species", "SH")
...
colnames(tax_tab_vsearch) <- c("Kingdom","Phylum","Class","Order","Family","Genus", "Species", "SH")
...
* Full-length reads are defined as reads with both ITS1catta and ITS2 primers - the 28S region of the read is trimmed in this analysis.
...
if (dim(mat_to_plot)[2] > 2) {
  ...
  mat_to_plot <- mat_to_plot[rowSums(mat_to_plot > 0.001, na.rm = TRUE) > 2, , drop = FALSE]

  # Check if there are any rows left after filtering
  if (nrow(mat_to_plot) > 0) {
    ...
    # Handle the case where no rows satisfy the condition
    cat("No rows satisfy the condition for plotting.")
  }
} else {
  # Handle the case where there are only 1 or 2 samples
  cat("Not enough samples for filtering.")
}
